...
export const processFileInBackground = async (file, chatId, dispatch, updateChatFile, addMessage) => {
  try {
    ...
    // Update the chat with the processed content
    dispatch(updateChatFile({
      chatId,
      fullText,
    ...
    dispatch(addMessage({
      ...
      message: {
        sender: 'ai',
        text: `❌ Sorry, I encountered an error while processing your ${file.type.startsWith('image/') ? 'image' : 'document'}: ${error.message}. Please try uploading the file again.`
      }
    }));
  }
};
